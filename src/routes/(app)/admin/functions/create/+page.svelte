<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_nst_ai_version ?? '0.0.0', WEBUI_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'NST-Ai version (v{{nst_ai_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						nst_ai_VERSION: WEBUI_VERSION,
						REQUIRED_VERSION: manifest?.required_nst_ai_version ?? '0.0.0'
					}
				)
			);
			return;
	...
	onMount(() => {
		window.addEventListener('message', async (event) => {
			if (
				!['https://NST-AI.com', 'https://www.NST-AI.com', 'http://localhost:9999'].includes(
					event.origin
				)
			)
				return;
