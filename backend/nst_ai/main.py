...
+===========================================================================+
                            Welcome to Vivek's Coding World!                 
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                "https://api.github.com/repos/NST-Ai/NST-Ai/releases/latest",
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for NST-Ai.
...
async def get_manifest_json():
    try:
        if app.state.EXTERNAL_PWA_MANIFEST_URL:
            try:
                response = requests.get(
                    ...
                    timeout=10,
                    headers={'User-Agent': 'NST-AI/1.0'}
                )
                response.raise_for_status()
                return response.json()
            except requests.exceptions.RequestException as e:
                log.error(f"Failed to fetch external manifest: {e}")
        ...
        return {
            ...
            "description": "NST-Ai is an open, extensible, user-friendly interface for AI that adapts to your workflow.",
            "start_url": "/",
            "display": "standalone",
        ...
        log.error(f"Error generating manifest.json: {e}")
...
async def serve_cache_file(
    ...
    try:
        ...
        log.error(f"Error serving cache file {path}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
