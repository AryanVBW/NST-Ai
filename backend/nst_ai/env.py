...
nst_ai_DIR = Path(__file__).parent  # the path containing this file
print(nst_ai_DIR)

BACKEND_DIR = nst_ai_DIR.parent  # the path containing this file
...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "NST-Ai")
if WEBUI_NAME != "NST-Ai":
    WEBUI_NAME += " (NST-Ai)"
...
if FROM_INIT_PY:
    PACKAGE_DATA = {"version": importlib.metadata.version("NST-Ai")}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data("nst_ai", "CHANGELOG.md") or b"").decode()
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv("DATA_DIR", nst_ai_DIR / "data")).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / "nst_ai_data", "zip", DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv("DATA_DIR", nst_ai_DIR / "data"))

STATIC_DIR = Path(os.getenv("STATIC_DIR", nst_ai_DIR / "static"))

FONTS_DIR = Path(os.getenv("FONTS_DIR", nst_ai_DIR / "static" / "fonts"))

FRONTEND_BUILD_DIR = Path(os.getenv("FRONTEND_BUILD_DIR", BASE_DIR / "build")).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(
        os.getenv("FRONTEND_BUILD_DIR", nst_ai_DIR / "frontend")
    ).resolve()
...
REDIS_KEY_PREFIX = os.environ.get("REDIS_KEY_PREFIX", "NST-Ai")
...
OTEL_SERVICE_NAME = os.environ.get("OTEL_SERVICE_NAME", "NST-Ai")
