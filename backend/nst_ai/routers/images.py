...
from nst_ai.config import CACHE_DIR
from nst_ai.constants import ERROR_MESSAGES
from nst_ai.env import ENABLE_FORWARD_USER_INFO_HEADERS, SRC_LOG_LEVELS
from nst_ai.routers.files import upload_file
from nst_ai.utils.auth import get_admin_user, get_verified_user
from nst_ai.utils.images.comfyui import (
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-NST-AI-User-Name"] = quote(user.name, safe=" ")
                headers["X-NST-AI-User-Id"] = user.id
                headers["X-NST-AI-User-Email"] = user.email
                headers["X-NST-AI-User-Role"] = user.role
