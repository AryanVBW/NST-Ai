...
from nst_ai.utils.payload import (
...
async def send_post_request(
    ...
    try:
        ...
        if r.ok is False:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"NST-Ai: Server Connection Error",
                )
        ...
        raise HTTPException(
            ...
            detail=detail if e else "NST-Ai: Server Connection Error",
        )
    finally:
        if not stream:
            await cleanup_response(r, session)
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = "NST-Ai: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
